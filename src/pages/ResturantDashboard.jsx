...
const RestaurantDashboard = () => {
  ...
  const [foodItems, setFoodItems] = useState(() => {
    // Check if we have saved items in localStorage
    const savedItems = localStorage.getItem('restaurantFoodItems');
    return savedItems ? JSON.parse(savedItems) : [
      { id: 1, name: "Fresh Bread", category: "Bakery", quantity: "10 loaves", expiry: "2023-06-15" },
      { id: 2, name: "Vegetable Platter", category: "Appetizers", quantity: "5 trays", expiry: "2023-06-14" },
      { id: 3, name: "Chicken Curry", category: "Main Course", quantity: "8 servings", expiry: "2023-06-13" },
    ];
  });
  ...
  React.useEffect(() => {
    localStorage.setItem('restaurantFoodItems', JSON.stringify(foodItems));
  }, [foodItems]);

  const addFoodItem = () => {
    if (!newFoodItem.name || !newFoodItem.quantity || !newFoodItem.expiry) return;
    
    if (editItem) {
      // Update existing item
      setFoodItems(foodItems.map(item => 
        item.id === editItem.id ? { ...newFoodItem, id: editItem.id } : item
      ));
    } else {
      // Add new item
      setFoodItems([
    ...
    <div className="min-h-screen bg-gradient-to-b from-green-50 to-emerald-50">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          ...
          >
            ...
            <div className="space-y-4">
              ...
              >
                {editItem ? "Update Food Item" : "Add to Inventory"}
              </motion.button>
            </div>
          </motion.div>
        </div>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          className="bg-gradient-to-r from-emerald-500 to-green-600 rounded-2xl shadow-lg p-8 text-white mb-16"
        >
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div className="text-center">
              ...
              <p className="text-xl">Volunteers Worked With</p>
            </div>
          </div>
        </motion.div>
      </div>
